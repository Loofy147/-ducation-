...
use computational_fundamentals::{
    algebraic_composability::{task_stats_monoid, TaskStats},
    causal_reasoning::{analyze_data, analyze_data_by_group, TreatmentData},
...
impl EdgeServer {
    ...
    fn process_task(&mut self, task: &Task, risk_tolerance: f64, use_optimized: Option<&str>) {
        if let Some(_result) = self.cache.get(&task.name) {
            ...
            self.stats = (task_stats_monoid().operation)(
                self.stats.clone(),
                TaskStats {
                    tasks_processed: 1,
                    data_processed: 0.0,
        ...
        if self.scheduler.schedule_task(task, risk_tolerance) {
            ...
            if let Some(task_type) = use_optimized {
                self.causal_data.push(TreatmentData {
                    ...
                    outcome,
                    confounding_variable: if task.value > 50.0 {
                        "Hard".to_string()
                    } else {
                        "Easy".to_string()
                    },
            ...
            self.cache
                .put(task.name.clone(), format!("result_{}", outcome));
...
fn main() -> Result<()> {
    println!(
        "{}",
        "🚀 Starting Full Edge Server Simulation".bold().yellow()
    );
    let budgets = Budgets {
        cpu: 1000.0,
        energy: 10000.0,
    ...
    for i in 0..101 {
        let task = Task {
            name: format!("scan_{}", i),
            operations: UncertainValue::new(1e8, 1e7),
            data_size: 1e7,
            network: false,
            value: 1.0,
        };
    ...
    for _ in 0..101 {
        let task = Task {
            name: "repeated_hotspot_task".to_string(),
            operations: UncertainValue::new(1e8, 1e7),
            data_size: 1e7,
            network: false,
            value: 1.0,
    ...
    println!(
        "Causal Analysis (Overall): Optimized={:.2}, Legacy={:.2}",
    ...
    for (group, (optimized, legacy)) in by_group {
        println!(
            "  Causal Analysis (Group {}): Optimized={:.2}, Legacy={:.2}",
